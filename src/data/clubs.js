export const clubsData = {
  "e-mentor": {
    id: "e-mentor",
    name: "E-Mentor",
    description:
      "Our mission at E-Mentor Club is to support students in improving their English-speaking skills through interactive and engaging sessions. We provide a platform where dedicated volunteers can mentor and guide learners, helping them build confidence in communication. Additionally, we create opportunities for members to connect with international students, fostering cross-cultural exchange and global collaboration. Join us to learn, teach, and grow together!",
    leaders: [
      {
        id: 1,
        name: "Rafiya Husenova",
        image: new URL("@/assets/img/leader-ementor.jpg", import.meta.url).href,
        age: 16,
        institution: "Navoi Presidential School ",
        score: "IELTS Band 8.0",
      },
    ],
    mentors: [
      {
        id: 1,
        name: "Viktoriia Kaminska",
        image: new URL("@/assets/img/mentor-1.jpg", import.meta.url).href,
        age: 19,
        institution: "Lviv Polytechnic National University",
        score: "C1",
        extra: "Linguistic education",
      },
      {
        id: 2,
        name: "Vazira Egamberdiyeva",
        image: new URL("@/assets/img/mentor-2.jpg", import.meta.url).href,
        age: 20,
        institution: "University of Minnesota,Twin Cities",
        score: "IELTS Band 8.0, SAT 1380",
        extra: 'A Scholar of "El-Yurt Umidi" Foundation',
      },
      {
        id: 3,
        name: "Bunyod Sattorov",
        image: new URL("@/assets/img/mentor-3.jpg", import.meta.url).href,
        age: 19,
        institution: "The University of Sydney",
        score: "IELTS Band 7.0",
        extra: 'A Scholar of "El-Yurt Umidi" Foundation',
      },
      {
        id: 4,
        name: "Ma'suma Mahammadjonova",
        image: new URL("@/assets/img/mentor-4.jpg", import.meta.url).href,
        age: 18,
        institution: "New Uzbekistan University",
        score: "IELTS Band 7.5, SAT 1490",
        extra: "2 years of volunteering experience in teaching English.",
      },
      {
        id: 5,
        name: "Abduaziz Ramazonov",
        image: new URL("@/assets/img/mentor-5.jpg", import.meta.url).href,
        age: 18,
        institution: "University of Minnesota,Twin Cities",
        score: "IELTS Band 7.5, EFSET C1",
        extra: 'A Scholar of "El-Yurt Umidi" Foundation',
      },
      {
        id: 6,
        name: "Madina Umarova",
        image: new URL("@/assets/img/mentor-6.jpg", import.meta.url).href,
        age: 18,
        institution: "Tashkent Medical Academy",
        score: "IELTS Band 8.0",
        extra: "Lumiere Research Alumni",
      },
      {
        id: 7,
        name: "Nusratov Izzatulla",
        image: new URL("@/assets/img/mentor-7.jpg", import.meta.url).href,
        age: 18,
        institution: "University of Birmingham",
        score: "IELTS Band 7.0, SAT 1440",
        extra: 'A scholar of "El-Yurt Umidi" Foundation',
      },
    ],
  },
  "no-exception": {
    id: "no-exception",
    name: "No Exception",
    description:
      "Our mission at No Exception! is to foster inclusivity and provide equal opportunities for everyone in our community. We aim to support students with limited abilities by organizing visits to their schools and hosting intellectual events. Through our efforts, we strive to create a more caring and supportive society where no one is left behind. Join us to make a difference and spread kindness!",
    past_events: [
      {
        id: 1,
        title: "World Poetry Day",
        date: "March 2023",
        description:
          'As part of the celebration of the 70th anniversary of the UNESCO Associated Schools Network, students of our program actively participated in the "Gift a Poem" initiative.Our members copied their chosen poems and gifted them to the visually impaired students of the Navoi region’s school.',
        images: [
          {
            id: 1,
            url: new URL(
              "@/assets/img/no_exception march_2023/1.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 2,
            url: new URL(
              "@/assets/img/no_exception march_2023/2.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 3,
            url: new URL(
              "@/assets/img/no_exception march_2023/3.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 4,
            url: new URL(
              "@/assets/img/no_exception march_2023/4.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 5,
            url: new URL(
              "@/assets/img/no_exception march_2023/5.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 6,
            url: new URL(
              "@/assets/img/no_exception march_2023/6.jpg",
              import.meta.url
            ).href,
          },
        ],
      },
      {
        id: 2,
        title: "Zakovat Competition",
        date: "November 2023",
        description:
          'During the visit, our members organized and competed in various intellectual games, including the "Zakovat" game, with the students of the school for disabled children.Additionally, to improve the students\' English language skills and enhance their fluency, they were provided with necessary language information and participated in "speaking sessions."',
        images: [
          {
            id: 1,
            url: new URL(
              "@/assets/img/no_exception november/1.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 2,
            url: new URL(
              "@/assets/img/no_exception november/2.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 3,
            url: new URL(
              "@/assets/img/no_exception november/3.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 4,
            url: new URL(
              "@/assets/img/no_exception november/4.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 5,
            url: new URL(
              "@/assets/img/no_exception november/5.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 6,
            url: new URL(
              "@/assets/img/no_exception november/6.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 7,
            url: new URL(
              "@/assets/img/no_exception november/7.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 8,
            url: new URL(
              "@/assets/img/no_exception november/8.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 9,
            url: new URL(
              "@/assets/img/no_exception november/9.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 10,
            url: new URL(
              "@/assets/img/no_exception november/10.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 11,
            url: new URL(
              "@/assets/img/no_exception november/11.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 12,
            url: new URL(
              "@/assets/img/no_exception november/12.JPG",
              import.meta.url
            ).href,
          },
          {
            id: 13,
            url: new URL(
              "@/assets/img/no_exception november/13.JPG",
              import.meta.url
            ).href,
          },
        ],
      },
    ],
    upcoming_events: [],
    leaders: [
      {
        id: 1,
        name: "Firdavs Xusenov",
        image: new URL("@/assets/img/leader-noexception.JPG", import.meta.url)
          .href,
        age: 16,
        institution: "Navoi Presidential School",
        score: "IELTS Band 7.5",
        extra: "Gold medalist in the regional Zakovat competition",
      },
    ],
  },
  "eco-light": {
    id: "eco-light",
    name: "Eco Light",
    description:
      "Our mission at ECO Light is to promote sustainability and environmental awareness by taking action to preserve the beauty of our surroundings. Through activities such as planting trees and flowers, we strive to enhance our community’s environmental health and create a greener, more vibrant space for everyone. Join us to protect nature and build a sustainable future!",
    past_events: [
      {
        id: 1,
        title: "Visit to the Family Health Centre",
        date: "November 2023",
        description:
          "Members visited the Family Health Centre, where, along with staff, they planted saplings in the large outdoor area and organized environmental cleanup activities.",
        images: [
          {
            id: 1,
            url: new URL(
              "@/assets/img/eco_light november/1.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 2,
            url: new URL(
              "@/assets/img/eco_light november/2.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 3,
            url: new URL(
              "@/assets/img/eco_light november/3.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 4,
            url: new URL(
              "@/assets/img/eco_light november/4.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 5,
            url: new URL(
              "@/assets/img/eco_light november/5.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 6,
            url: new URL(
              "@/assets/img/eco_light november/6.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 7,
            url: new URL(
              "@/assets/img/eco_light november/7.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 8,
            url: new URL(
              "@/assets/img/eco_light november/8.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 9,
            url: new URL(
              "@/assets/img/eco_light november/9.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 10,
            url: new URL(
              "@/assets/img/eco_light november/10.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 11,
            url: new URL(
              "@/assets/img/eco_light november/11.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 12,
            url: new URL(
              "@/assets/img/eco_light november/12.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 13,
            url: new URL(
              "@/assets/img/eco_light november/13.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 14,
            url: new URL(
              "@/assets/img/eco_light november/14.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 15,
            url: new URL(
              "@/assets/img/eco_light november/15.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 16,
            url: new URL(
              "@/assets/img/eco_light november/16.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 17,
            url: new URL(
              "@/assets/img/eco_light november/17.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 18,
            url: new URL(
              "@/assets/img/eco_light november/18.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 19,
            url: new URL(
              "@/assets/img/eco_light november/19.jpg",
              import.meta.url
            ).href,
          },
        ],
      },
    ],
    upcoming_events: [],
    leaders: [
      {
        id: 1,
        name: "Karimov Rustamjon",
        image: new URL("@/assets/img/leader-ecolight-1.jpg", import.meta.url)
          .href,
        age: 16,
        institution: "Navoi Presidential School",
        score: "IELTS Band 7.5",
        extra: "'Yosh kitobxon' competition:  3rd place in region",
      },
      {
        id: 2,
        name: "Sardor Shirinboyev",
        image: new URL("@/assets/img/leader-ecolight-2.jpg", import.meta.url)
          .href,
        age: 16,
        institution: "Navoi Presidential School",
        score: "IELTS Band 7.0",
        extra: 'Participant of the program "Leaders of Tomorrow 2024"',
      },
    ],
  },
  "senior-care": {
    id: "senior-care",
    name: "Senior Care",
    description:
      "Our mission at Senior Care is to honor and appreciate the experiences of World War II veterans by learning their stories, expressing gratitude, and giving meaningful gifts. We strive to connect with these heroes, acknowledge their sacrifices, and preserve their legacy for future generations. Join us to give back and show appreciation for those who shaped history!",
    past_events: [
      {
        id: 1,
        title: "Visit to the World War II Veteran",
        date: "January 2024",
        description:
          "A visit was organized to check on the well-being of a World War II veteran. During the visit, members listened to excerpts about the war and the veteran's memories from that time, gaining insights into the events and developing a deeper understanding of patriotism. As a token of gratitude, gifts were presented to the veteran.",
        images: [
          {
            id: 1,
            url: new URL(
              "@/assets/img/senior_care january_2024/1.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 2,
            url: new URL(
              "@/assets/img/senior_care january_2024/2.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 3,
            url: new URL(
              "@/assets/img/senior_care january_2024/3.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 4,
            url: new URL(
              "@/assets/img/senior_care january_2024/4.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 5,
            url: new URL(
              "@/assets/img/senior_care january_2024/5.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 6,
            url: new URL(
              "@/assets/img/senior_care january_2024/6.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 7,
            url: new URL(
              "@/assets/img/senior_care january_2024/7.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 8,
            url: new URL(
              "@/assets/img/senior_care january_2024/8.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 9,
            url: new URL(
              "@/assets/img/senior_care january_2024/9.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 10,
            url: new URL(
              "@/assets/img/senior_care january_2024/10.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 11,
            url: new URL(
              "@/assets/img/senior_care january_2024/11.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 12,
            url: new URL(
              "@/assets/img/senior_care january_2024/12.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 13,
            url: new URL(
              "@/assets/img/senior_care january_2024/13.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 14,
            url: new URL(
              "@/assets/img/senior_care january_2024/14.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 15,
            url: new URL(
              "@/assets/img/senior_care january_2024/15.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 16,
            url: new URL(
              "@/assets/img/senior_care january_2024/16.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 17,
            url: new URL(
              "@/assets/img/senior_care january_2024/17.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 18,
            url: new URL(
              "@/assets/img/senior_care january_2024/18.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 19,
            url: new URL(
              "@/assets/img/senior_care january_2024/19.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 20,
            url: new URL(
              "@/assets/img/senior_care january_2024/20.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 21,
            url: new URL(
              "@/assets/img/senior_care january_2024/21.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 22,
            url: new URL(
              "@/assets/img/senior_care january_2024/22.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 23,
            url: new URL(
              "@/assets/img/senior_care january_2024/23.jpg",
              import.meta.url
            ).href,
          },
          {
            id: 24,
            url: new URL(
              "@/assets/img/senior_care january_2024/24.jpg",
              import.meta.url
            ).href,
          },
        ],
      },
    ],
    upcoming_events: [],
    leaders: [
      {
        id: 1,
        name: "Kuchkaraliyev Muhammad Rizo Fuzayl",
        image: new URL("@/assets/img/leader-seniorcare.jpg", import.meta.url)
          .href,
        age: 17,
        institution: "Navoi Presidential School",
        score: "IELTS Band 6.0",
        extra: "Elite athlete",
      },
    ],
  },
  "more-to-give": {
    id: "more-to-give",
    name: "More To Give",
    description:
      "Our mission at More to Give is to encourage generosity and community engagement by providing opportunities to donate books, time, skills, and resources. Through volunteer efforts, we aim to address social, environmental, and humanitarian issues while fostering a spirit of empathy and giving. Join us to share, support, and create a positive impact!",
    past_events: [],
    upcoming_events: [],
    leaders: [
      {
        id: 1,
        name: "Shavkatullayev Muhammad",
        image: new URL("@/assets/img/leader-moretogive-1.jpg", import.meta.url)
          .href,
        age: 16,
        institution: "Navoi Presidential School",
        score: "IELTS Band 7.5",
        extra: "School President",
      },
      {
        id: 2,
        name: "Dilnura Hamidova",
        image: new URL("@/assets/img/leader-moretogive-2.png", import.meta.url)
          .href,
        age: 16,
        institution: "Navoi Presidential School",
        score: "IELTS Band 7.0",
        extra: "Vice President at school",
      },
    ],
  },
};
